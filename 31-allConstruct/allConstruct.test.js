...
describe('Testing AllConstruct', () => {
	it('Should return 2D array', () => {
		expect(Array.isArray(allConstruct('', ['a', 'b']))).toBe(true);
		expect(Array.isArray(allConstruct('', ['a', 'b'])[0])).toBe(true);
	});
	it('Should return [ [ "ab", "c" ] ]', () => {
		const res = allConstruct('abc', ['ab', 'c']);
	...
	it("Should return [ [ 'purp', 'le' ], [ 'p', 'ur', 'p', 'le' ] ]", () => {
		const res = allConstruct('purple', ['purp', 'p', 'ur', 'le', 'purpl']);
	...
	it('Should return [[]]', () => {
		const res = allConstruct('', ['a', 'c']);
		expect(res.length).toBe(1);
		expect(res[0].length).toBe(0);
	});
	it('Should return []', () => {
		const res = allConstruct('aaaaaaaaaaaaaaaaaaaaaaaabc', ['aa', 'a', 'aaa', 'aa', 'aaaa']);
